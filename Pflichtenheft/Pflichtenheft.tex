...
\section{Glossar}
\begin{itemize}
\item Anti Aliasing: Verhindern von Kanten in Bildern
\item Anisotropes Filtern: Methode um den Schärfeeindruck von entfernten Texturen zu erhalten
\item JAVA: Hier verwendete Programmiersprache
\item Jogl: (JAVA OpenGL) In JAVA eingebettetes OpenGL
\item LATEX: Textverarbeitungsprogramm
\item OpenStreetMap: Projekt das kostenlos Kartenmaterial zur Verfügung stellt
\item OpenGL: Plattform- und Programmiersprachen unabhängige Schnittstelle zur Entwicklung von 2D- und 3D-Computergrafik
\item Overlay: Keine Ahnung ob ich das erklären soll???
\item Plattformunabhängigkeit: Fähigkeit auf verschiedenen Systemen zu laufen, unabhängig von der Wahl des Betriebssystems
\item POI: (Point Of Interest) interessanter Punkt auf der Karte
