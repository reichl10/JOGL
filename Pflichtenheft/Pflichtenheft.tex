...
Die grafische Benutzeroberfläche zeigt dafür eine Weltkugel, die frei gedreht und gezoomt werden kann. Die Steuerung erfolgt hierbei mit Tastatur und Maus. Als Oberflächentextur können hierbei verschiedene Typen wie Satellitenbilder der NASA oder Kartendaten des OpenStreetMap-Projekts ausgewählt werden. 
...
\begin{figure}[!htb]
	\centering
	\includegraphics[scale=0.1]{Kinder-Weltkarte.jpg}
...
\begin{figure}
	\centering
	\includegraphics[scale=0.9]{GUI-Ansicht-Welt.png}
	\caption{Die Benutzeroberfläche mit der Ansicht auf die Weltkugel}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[scale=0.9]{GUI-Ansicht-Kinder-Weltkarte.png}
	\caption{Die Benutzeroberfläche mit der Kinder-Weltkarte}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[scale=0.9]{GUI-Ansicht-Strassenkarte.png}
...
\section{Glossar}
\begin{itemize}
\item Anisotropes Filtern: Methode um den Schärfeeindruck von entfernten Texturen zu erhalten
\item Antialiasing: Methode zur Kantenglättung in Grafiken
\item API: Application Programming Interface, englische Bezeichnung für Anwendungsprogrammierschnittstelle
\item Detailfenster: Zeigt alle Details zu einem markierten Punkt
\item Eclipse: Software-Plattform und Integrierte Entwicklungsumgebung
\item GIT: Versionsverwaltungssoftware
\item GUI: (Graphical User Interface) Grafische Benutzeroberfläche
\item Java: Verwendete Programmiersprache
\item Javadoc: Dokumentation von Klassen und Methoden in Java
\item Jogl: (Java OpenGL) OpenGL API in Java
\item \LaTeX: Textsatzprogramm
\item OpenGL: API zur Entwicklung von 2D- und 3D-Computergrafik
\item OpenStreetMap: Projekt, das freies Kartenmaterial zur Verfügung stellt
\item Overlay: Bezeichnet die Text- oder Symboleinblendungen auf den Karten bzw. auf der Weltkugel
\item Plattformunabhängigkeit: Das Programm kann auf diversen Betriebssystemen ausgeführt werden ohne entsprechende Anpassung vorzunehmen.
\item POI: (Point Of Interest) POIs sind Orte, die für den Nutzer einer Karte oder eines Navigationssystems Bedeutung haben könnten, wie z.B. Tankstellen, Banken, Tierparks.
\item RAM: Arbeitsspeicher
\item Rendern: ??
\item Server: Stellt die Kartendaten zur Verfügung
\item Textur: Oberflächengrafik für 3D-Modelle
