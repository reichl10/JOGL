...
\end{lstlisting}
Bildet die Schnittstelle für einen Cache, der Methoden zum Hinzufügen, Abfragen und Löschen von Daten bietet. Die Verwaltung der Größenbeschränkungen ist Sache des \textit{RequestDistributors}.\\
\subsubsection*{Zusammenfassung der wichtigsten Methoden}
\begin{itemize}
...
interface Geometry
\end{lstlisting}
Abstrahiert den Teil der Sichtbarkeits- und Positionsberechnungen, der von dem Oberflächenmodell (wie Globus oder Kartenebene) abhängt. Die Kamera besitzt ein solches Geometry-Objekt, um sich dem momentanen Ansichtsmodus entsprechend zu verhalten.\\
...
\end{lstlisting}
Eine Implementierung dieses Interfaces kann asynchrone Antworten zur \textit{requestObject()}-Anfrage an eine \textit{Source} entgegen nehmen.\\
...
\begin{itemize}\item\texttt{main()} Startet das Programm.
\end{itemize}
...
\begin{itemize}
\item \texttt{getVisibleTiles()}
\item \texttt{getProjectionMatrix()}
\end{itemize}
...
\begin{itemize}
\item \texttt{getInteger()}
\item \texttt{putInteger()}
\end{itemize}
...
\begin{itemize}
\item \texttt{static void loadSettings()}
\item \texttt{static void setDefaultSettings()}
\end{itemize}
...
\begin{itemize}
\item \texttt{static byte[] get()}
\item \texttt{static byte[] post()}
\end{itemize}
...
\begin{itemize}
\item\texttt{float getHeight(GeoCoordinates g)}
\end{itemize}
...
Es teilt die Funktionalität der Komponenten in Datenmodell (\textit{Model}), Präsentation (\textit{View}) und Steuerung (\textit{Controller}) ein. 

Das Datenmodell beinhaltet die zugrundeliegende Datenverwaltung und  Berechnungsmodelle, die Präsentation ist für die Schnittstelle zum Nutzer verantwortlich, während der Controller Eingaben entgegennimmt und die View oder die Views steuert.
