...
/**
 * Recieves requests for objects, can get the objects from one Source that can be set. 
 * Stores requested objects in caches to minimize loading-time.
 * 
 * 
 */
public class RequestDistributor<Key, Value> implements Source<Key, Value> {
     ...
     * size of entry, identifier of entry (key), measure for time passed since last use
     * Returns hashcode of entry determined by key.
     * 
     */
    ...
    /**
     * Tries to return requested object. Consults caches first and updates hashLastUsed
     * and entry in <code>tree</code> if object found in cache.
     * When found in other cache than primary cache, put object in primary cache.
     * If object not found in caches, consult <code>Source</code>, add to pendingRequests
     * and return null with responseType ASYNCHRONE.
     * If no Source set, return null and responseType MISSING instead.
     * 
     ...
     * and notifies listeners that requested object is available now.
     * Method wont do anything when parameter is null or Key is already represented in cache.
     * 
     ...
     * When argument is invalid, this method does nothing.
     * 
    ...
    /**
     * Erases Key from cache1 and puts it in cache2.
     * Updates <code>tree</code>, hashLastUsed and freeCacheSpace of both caches.
     * If cache2 is null or invalid, method will return true and the entry will be lost. 
     * If Key or cache1 aren't valid, method will do nothing but return true. 
