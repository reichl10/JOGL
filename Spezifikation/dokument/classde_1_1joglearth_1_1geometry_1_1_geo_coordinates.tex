...
Coordinates are given in the format {\ttfamily /[0-\/9]?[0-\/9]$\ast$°$\ast$([0-\/9]?[0-\/9]$\ast$'($\ast$[0-\/9]?{\tt 0-\/9}?$\ast$\char`\"{})?)?$|$[0-\/9]+(.[0-\/9]+)?/$<$/code$>$
...
@param lat The latitude, in the format $<$code$>$\textbackslash{}s$\ast$(coord)\textbackslash{}s$\ast$[\-N\-S]?\textbackslash{}s$\ast$$<$/code$>$, e.\-g. 55° 17' 48.\-2\char`\"{} N \\
\hline
\end{DoxyParams}
